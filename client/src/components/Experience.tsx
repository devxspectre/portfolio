  ...
  const experiences = [
    {
      ...
      description: `
        • Engineered robust backend APIs using Node.js and Express for real-time communication, supporting AI-driven features
        and handling high-concurrency requests with 99% uptime.
        • Developed over 30 reusable React components for AI workflows, while optimizing backend endpoints to handle upto 1000+ daily queries with 95% accuracy using LangChain and OpenAI APIs, Created custom Webpack configurations for embeddable widgets and chatbot integrations, minimizing bundle sizes by
          ...
          {experiences.map((exp, index) => (
            ...
            >
              ...
              <div className="pl-4 border-l-2 border-gray-600">
                <div className="text-gray-200 text-lg leading-relaxed mono-font space-y-2">
                  {exp.description.split("\n").map((paragraph, idx) => {
                    // Check if the paragraph contains bullets
                    if (paragraph.includes("•")) {
                      // Split by bullets and create a list
                      const parts = paragraph.split("•");
                      const firstPart = parts.shift(); // The part before the first bullet (if any)
                      ...
                      return <div key={idx}>{paragraph.trim()}</div>;
                    }
                  })}
                </div>
              </div>
